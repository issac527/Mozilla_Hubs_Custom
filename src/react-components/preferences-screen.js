...
const preferenceLabels = defineMessages({
  ...
  snapRotationDegrees: {
    ...
    defaultMessage: "마우스 민감도 설정(0 ~ 90)"
  },
  disableMovement: {
  ...
  invertTouchscreenCameraMove: {
    ...
    defaultMessage: "카메라 이동 방향 반전 - 터치스크린용"
  },
  locale: {
  ...
  render() {
    return (
      <div ref={this.ref} className={styles.toast}>
        <div className={styles.row}>
          ...
          <div className={styles.refreshPrompt}>
            <FormattedMessage
              ...
              defaultMessage="기본 설정은 저장되지만 일부 변경 사항은 페이지를 새로 고칠 때까지 적용되지 않습니다."
            />
          </div>
        ...
        >
          <FormattedMessage id="preferences-screen.refresh-now" defaultMessage="새로 고침" />
        </button>
      </div>
    );
  }
          ...
          {TOP_LEVEL_CATEGORIES.map(category => (
            <NavItem
              ...
              ariaLabel={intl.formatMessage(
                { id: "preferences-screen.select-category ", defaultMessage: "카테고리 선택 {categoryName}" },
                {
